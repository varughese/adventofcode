const input = `................458...689.556..3............197......582........720.........................515..352..286.........670.741.....895.626.......
...
const isSymbol = (token) => !numeric(token) && token !== '.'
...
let parsingNumber = true;
for (let row=0; row<grid.length; row++) {
	for (let col=0; col<grid[row].length; col++) {
		const around = directions.map(coords => [coords[0] + row,coords[1] + col]).filter(coords => (coords[0] !== 0 && coords[1] !== 0) && coords[0] >= 0 && coords[1] >= 0 && (coords[0] < grid.length && coords[1] < grid[row].length))

		const symbols = around.filter(coords => {
			const adjacent = grid[coords[0]][coords[1]]
			return isSymbol(adjacent)
		})

		const token = grid[row][col]

		if (!currentNumberParse.parsing && numeric(token)) {
			currentNumberParse.parsing = true;
			currentNumberParse.rowStart = row;
			currentNumberParse.colStart = col;
		}

		if (currentNumberParse.parsing) {
			if (numeric(token)) {
				currentNumberParse.number += token;
				if (symbols.length > 0) {
					currentNumberParse.symbols.push(...symbols)
				}
			} else {
				currentNumberParse.number = Number(currentNumberParse.number)
				parsed.push({...currentNumberParse})
				currentNumberParse.symbols= []
				currentNumberParse.number= ''
				currentNumberParse.parsing= false;
			}
		}
	}
}
...
// Part 1
const parsedNumbers = input.split('\n').map(line => line.replace(/[^\d]/g, '.').split('.').filter(x => x !== '').map(Number)).flat()

// const nonPartNumbers = numbers;

console.log('part 1')
console.log("all numbers", sum(parsedNumbers));
// console.log("non part numbers", sum(numbers))
// console.log("difference", sum(parsedNumbers) - sum(numbers));

console.log('part 2')
const allNumbers = parsed.map(p => p.number)
const partNumbers = parsed.flatMap(p => p.symbols.length === 0 ? [] : [p.number])
console.log("part numbers", sum(partNumbers))
...
parsed.forEach(number => {
	...
	number.symbols.forEach(symbols => {
		const key = symbols[0] + ',' + symbols[1]
		if (touchedKeys.has(key)) {
			return;
		}
		touchedKeys.add(key)
		const token = grid[symbols[0]][symbols[1]]
		if (token === '*') {
			gearMap[key] = gearMap[key] || []
...
let gearRatioSum = 0
Object.values(gearMap).forEach(gearNumbers => {
	console.log(gearNumbers)
	if (gearNumbers.length === 2) {
		gearRatioSum += gearNumbers[0].number * gearNumbers[1].number
	}
})
